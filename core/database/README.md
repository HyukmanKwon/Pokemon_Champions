...
접속 정보는 환경변수로 바꾼다. 기본값은 `db.py` 참고.
...
```bash
PGDATABASE=pokemon_test python main.py     # 다른 DB에 구축
```
...
```
database/
├── main.py               진입점. SQL 생성 → 실행을 순서대로 수행
├── db.py                 접속 설정, 경로 상수, connect()
...
│   └── moves.py          기술 플래그
│
├── overrides/            재구축해도 살아남는 수정값 (git 커밋 대상)
│   ├── move_flags.json      기술 플래그
│   └── move_ko_names.json   PokeAPI 가 옛 번역을 주는 한국어 이름
│
├── cache/                내려받은 PokeAPI CSV (재생성 가능. 커밋 불필요)
│   └── move_flag_map.csv
│
...
│
└── sql/                  main.py가 만든 SQL 파일들 (실행 시 생성)
```

`sql/` 안의 파일은 실행할 때마다 덮어쓰인다. 손으로 고쳐도 다음 실행에서 사라진다.

---

## 3. 구축 순서

main.py는 아래 순서로 **"SQL 생성 → 즉시 실행"** 을 반복한다.
...
```bash
cd database
python annotator/moves.py    # 브라우저가 열린다 (http://localhost:8765)
```
...
```bash
python check_moves.py 목록.txt      # 한국어 이름 한 줄에 하나
```
...
`main.py` 에는 삭제 기능이 없다. psql 에서 직접 지운다.
...
확인은 `\dt` 와 `\dT` 로 한다. 둘 다 비어 있으면 `python main.py` 를 다시 돌린다.
...
3. `main.py` 의 `STEPS` 에 의존 순서에 맞게 끼워 넣는다
4. `schema.py` 의 `ALL_TABLES` 에 테이블 이름을 추가한다

파일에 직접 쓰지 않고 문자열을 반환하는 것이 규칙이다. 파일 배치는 main.py가 한다.
각 `get_*.py` 의 `main()` 은 그 파일을 단독 실행할 때만 쓰는 진입점이고,
`main.py` 는 `build()` 만 호출한다.
