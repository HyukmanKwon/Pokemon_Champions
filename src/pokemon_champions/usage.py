"""채용률·메타 — championsbattledata.com 에서 받아 data/cache/usage/ 에 캐시한다.

── 왜 이 층에 있나 ──
  assets.py 와 같은 자리다. 프로젝트 바깥에서 받아 캐시하는 일이고 DB 를
  보지 않는다. 한국어로 바꾸고 DB 와 맞추는 것은 services/meta.py 가 한다.
...
def tokens(name):
    ...
    services/meta.py 도 쓴다 — 함께 쓰는 포켓몬 목록이 저쪽 표기로 오므로
